...
This script runs the model for the baseline environmental layer.

Usage:
    python calculate_province_area_projections_baseline.py
...
def predict_provinces():
    ...
    # Loading global data
    global_scenario = f"/data/gpfs/projects/punim2700/data/share/global_scenarios/baseline_env_data.nc"
    global_scenario = xr.open_dataset(global_scenario).squeeze()
    X = global_scenario.to_dataframe().dropna(how="any")
    ...
    y_hat = pd.Series(y_hat, index=X.index, name="baseline_provinces")
    
    y_hat.to_csv(f"/data/gpfs/projects/punim2700/outputs/global_province_predictions/baseline_predicted_provinces.csv")
